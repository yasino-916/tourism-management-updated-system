    ...
    visitorService.getSiteById(id).then(s => {
      if (s) {
        const mappedSite = {
          id: s.site_id,
          name: s.site_name,
          images: (s.image_url || s.image) ? [s.image_url || s.image] : ['https://via.placeholder.com/800x400'],
          location: s.location_address || s.location || 'Unknown',
          description: s.full_description || s.description || 'No description available.',
          attractions: (s.nearby_attractions ? s.nearby_attractions.split(',').map(a => a.trim()) : []),
          price: s.visit_price || s.price || 0,
          guideFee: s.guide_fee || 0,
          duration: s.estimated_duration || s.visit_duration || '3-4 hours',
          reviews: []
        };
        setSite(mappedSite);
                  ...
                  <div className="d-flex gap-2 overflow-auto">
                    {site.images.map((img, idx) => (
                      <img
                        ...
                        onError={(e) => {
                          e.target.onerror = null;
                          e.target.src = 'https://via.placeholder.com/100?text=Err';
                        }}
                      />
                    ))}
                  </div>
